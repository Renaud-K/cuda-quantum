...
public:
  ...
  qudit() : idx(cudaq::execution_manager::allocateQudit(n_levels())) {}
  ...
  qudit(const state &state) : qudit() {
    ...
    cudaq::execution_manager::initializeState(v, state);
  }
  ...
  ~qudit() { cudaq::execution_manager::returnQudit({n_levels(), idx}); }
};
